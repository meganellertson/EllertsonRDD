...
##
library(rdd)
weightsA <- rdd::kernelwts(RDD7$bac7, center = 0, bw = 0.05, kernel = "rectangular")
C1PA <- lm(recidivism ~ bac7 + aged + white + male + acc, 
            data = RDD7,
            weights = weightsA)
...
C3PA <- lm(recidivism ~ hac7*bac7*dui7 + aged + white + male + acc, 
            data = RDD7,
            weights = weightsA)
lmtest::coeftest(C2PA, vcov = sandwich::vcovHC(C2PA))
```
